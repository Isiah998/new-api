...
// are exposed separately for self-service flows driven from qing-api's own UI.
...
func WaffoPancakeBuyerIdentityFromUserID(userID int) string {
	return fmt.Sprintf("qing-api-user-%d", userID)
}
...
const (
	defaultWaffoPancakeStoreName   = "qing-api-store"
	defaultWaffoPancakeProductName = "qing-api-charge-product"
)
...
//
// OnetimeProduct (not SubscriptionProduct) because qing-api has no renewal-
// event handling; Pancake auto-renewing without qing-api extending user
