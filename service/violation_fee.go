...
func HasCSAMViolationMarker(err *types.QingAPIError) bool {
	if err == nil {
		return false
	}
...
func WrapAsViolationFeeGrokCSAM(err *types.QingAPIError) *types.QingAPIError {
	if err == nil {
		return nil
	}
...
func NormalizeViolationFeeError(err *types.QingAPIError) *types.QingAPIError {
	if err == nil {
		return nil
	}
...
func shouldChargeViolationFee(err *types.QingAPIError) bool {
	if err == nil {
		return false
	}
...
func ChargeViolationFeeIfNeeded(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, apiErr *types.QingAPIError) bool {
