...
面板鉴权采用短期 Access Token、HttpOnly Refresh Cookie 与服务端登录会话控制面的组合。面板请求不再依赖 Gin session，也不再要求 `Qing-Api-User` 请求头。
...
`User.AccessToken`（面板 PAT）继续支持 `Authorization: Bearer <pat>`，也兼容原有的单值 `Authorization: <pat>`。`Qing-Api-User` 不再参与鉴权，外部脚本不需要再发送 Bearer 与用户 ID 双请求头。这是有意的调用契约简化；旧 PAT 本身无需重新生成。

PAT 不是浏览器登录会话，不能调用登录会话管理接口、查看/批量导出中转令牌、轮换面板 PAT，也不能签发绑定具体登录会话的 Security Proof。这些密钥操作必须由有效的浏览器登录会话发起。

## 首次初始化保护

首次初始化会用固定主键的数据库事务同时写入初始化记录、Root 用户和运行模式，多个进程或并发请求只能有一个成功。回环或受信任私网客户端可以直接初始化；公网客户端必须先配置高熵 `SETUP_TOKEN`，再在初始化向导中提供相同值。`POST /api/setup` 同时受 Critical IP 限流保护。

容器或公网反代部署建议在首次启动前生成至少 32 字节随机值，例如：

```bash
openssl rand -hex 32
```

完成初始化后可以移除 `SETUP_TOKEN` 并重启。请确保 `TRUSTED_PROXIES` 只包含实际反向代理地址；如果将不可信私网客户端误判为可信来源，会削弱无 `SETUP_TOKEN` 时的来源校验。

## 查询参数中的中转令牌

Gemini 兼容接口仍支持 `?key=...`，但应用在鉴权后会立即从请求 URL 中删除该参数，内置访问日志也只记录 URL path。更推荐使用 `x-goog-api-key` 或 `Authorization` 请求头。

反向代理的访问日志发生在请求进入应用之前，应用无法替代理清理。Nginx 等反代应避免记录包含查询字符串的 `$request_uri`，可改记 `$uri`；日志目录还应限制为仅服务账号和运维人员可读。
...
- 自建客户端应按新的 AuthBundle、`flow_token` 和 Security Proof 契约升级；PAT 客户端可直接移除 `Qing-Api-User`。
