...
// 上游地址
const (
	upstreamModelsURL  = "https://basellm.github.io/llm-metadata/api/qingapi/models.json"
	upstreamVendorsURL = "https://basellm.github.io/llm-metadata/api/qingapi/vendors.json"
)
...
func getUpstreamURLs(locale string) (modelsURL, vendorsURL string) {
	...
	if l, ok := normalizeLocale(locale); ok && l != "" {
		return fmt.Sprintf("%s/api/i18n/%s/qingapi/models.json", base, l),
			fmt.Sprintf("%s/api/i18n/%s/qingapi/vendors.json", base, l)
	}
	return fmt.Sprintf("%s/api/qingapi/models.json", base), fmt.Sprintf("%s/api/qingapi/vendors.json", base)
}
